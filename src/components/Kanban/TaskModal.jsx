...
import { generateDescription } from '../../api/claudeApi';
...
function TaskModal({ task, onClose }) {
  ...
  const handleStatusChange = (newStatus) => {
    ...
    logActivity(user?.name || 'Someone', `changed status of "${task.title}" to ${newStatus}`);
  ...
  const handleAssigneeChange = (newAssignee) => {
    ...
    logActivity(user?.name || 'Someone', `assigned "${task.title}" to ${newAssignee || 'Unassigned'}`);
    if (newAssignee) {
      addNotification(`"${task.title}" has been assigned to ${newAssignee}`, 'info');
    }
      ...
      <div className="fullpage-modal">
        ...
        <div className="fullpage-topbar">
          <div className="fullpage-topbar-left">
            <span className="modal-task-id">TASK-{task.id.toString().slice(-4)}</span>
          </div>
          <div className="fullpage-topbar-right">
            <button className="modal-delete-btn" onClick={handleDelete}>Delete</button>
            <button className="modal-close-x" onClick={onClose}>✕ Close</button>
          </div>
        </div>
        ...
        <div className="fullpage-body">
          ...
          <div className="fullpage-left">
            ...
            <div className="modal-field">
              <div className="modal-desc-header">
                ...
                <button className="ai-mini-btn" onClick={handleAIGenerate} disabled={loading}>
                  {loading ? '...' : '🤖 AI Generate'}
                </button>
              </div>
              <textarea
            ...
            <AttachmentSection taskId={task.id} taskTitle={task.title} />
          ...
          <div className="fullpage-right">
            ...
            <div className="sidebar-field">
              <label>Task ID</label>
              <div className="sidebar-readonly">TASK-{task.id.toString().slice(-4)}</div>
            </div>
            ...
            <div className="sidebar-field">
              ...
              <div className="sidebar-readonly">
                {new Date(parseInt(task.id) || Date.now()).toLocaleDateString('en-GB', {
