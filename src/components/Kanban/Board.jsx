...
function Board() {
  ...
  const handleDragEnd = (event) => {
    ...
    if (over && active.id !== over.id) {
      ...
      logActivity(user?.name || 'Someone', `moved "${movedTask?.title}" to ${over.id}`);
    }
  };
  ...
  const handleAddTask = (e) => {
    ...
    addTask({ title, description, status: 'todo', dueDate, projectId: selectedProjectId, assignee: '' });
    logActivity(user?.name || 'Someone', `created task "${title}" in ${selectedProject?.name}`);
    setTitle('');
    setDescription('');
    setDueDate('');
    setShowForm(false);
  ...
  useEffect(() => {
    ...
    tasks.filter((t) => t.projectId === selectedProjectId).forEach((t) => {
      if (t.dueDate && t.status !== 'done' && new Date(t.dueDate) < today) {
        if (!alreadyNotified.includes(t.id)) {
          addNotification(`Task "${t.title}" is overdue!`, 'warning', t.id);
  ...
  }, [focusTaskId, tasks, selectedProjectId, setSelectedProjectId, setFocusTaskId]);

  const handlePrioritize = async () => {
    setLoading(true);
    const result = await prioritizeTasks(filteredTasks);
    setPriorityResult(result);
    setLoading(false);
    if (result) showToast('AI prioritization complete', 'success');
  };

  return (
    <div className="board-container">
      <div className="board-header">
        <h2>{selectedProject ? `${selectedProject.name} — Board` : 'Select a Project'}</h2>
        <div>
      ...
      {!selectedProjectId ? (
        <p className="chart-empty">Create or select a project from the sidebar to get started.</p>
      ) : (
        <>
